...
function deleteSql($table, $condition){
	...
	if($condition==='' || $condition===NULL){
		throw new Comm_Exception_Program('Should enter delete conditions');
	}
	//combine sql
	foreach ($condition as $ckey => $cvalue) {
		$conditiontemp[] = '`'.$ckey."`='".$cvalue."'";
 ...
 * @param type $data two dimensional array
 * @return type
 * @throws Comm_Exception_Program
 * 
 */
function insertMultiSql($table, $data){
	$db = DB();
	if($table===NULL || $table === ''){
		throw new Comm_Exception_Program('Should enter a table name');
	}
